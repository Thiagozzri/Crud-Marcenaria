...
const getErrorMessage = (error) => {
  ...
  if (error.message.toLowerCase().includes("network")) {
    return "Não foi possível acessar o Supabase. Confira sua internet e os dados de conexão.";
  }
...
function MarcenariaApp() {
  ...
  const requireConfiguration = () => {
    ...
    Alert.alert(
      ...
      "No Snack, informe a URL e a chave pública em src/config.js.",
    );
    return false;
  };
  ...
  const handleUpdate = async (material, input) => {
    if (!requireConfiguration()) return;
    setBusy(true);
    try {
