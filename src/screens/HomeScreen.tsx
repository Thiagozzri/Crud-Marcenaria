...
import type { EstoqueStatus, Material } from '../types/material';
import { formatCurrency, getEstoqueStatus, getValorTotal } from '../utils/material';

type StatusFilter = 'Todos' | EstoqueStatus;
...
const statusFilters: StatusFilter[] = ['Todos', 'Disponível', 'Estoque baixo', 'Sem estoque'];
...
export function HomeScreen({
  ...
  const categories = useMemo(
    () => ['Todas', ...Array.from(new Set(materials.map((item) => item.categoria))).sort()],
    [materials],
  );
  ...
  const filteredMaterials = useMemo(() => {
    ...
    return materials.filter((material) => {
      ...
      const matchesCategory = category === 'Todas' || material.categoria === category;
      const materialStatus = getEstoqueStatus(material.estoque, material.estoque_minimo);
      const matchesStatus = status === 'Todos' || materialStatus === status;
      return matchesSearch && matchesCategory && matchesStatus;
    });
  }, [category, materials, search, status]);

  const totalValue = materials.reduce((sum, material) => sum + getValorTotal(material), 0);
  const lowStockCount = materials.filter(
    (material) => getEstoqueStatus(material.estoque, material.estoque_minimo) !== 'Disponível',
  ).length;
  ...
  const header = (
    <View>
      <View style={styles.topBar}>
        ...
        <Pressable accessibilityLabel="Atualizar materiais" onPress={onRefresh} style={styles.refreshButton}>
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.heroStats}>
          ...
          <View style={styles.heroStat}>
            <Text style={[styles.heroStatValue, lowStockCount > 0 && styles.alertValue]}>
              {lowStockCount}
            </Text>
          ...
          <View style={styles.verticalDivider} />
          <View style={styles.heroStat}>
            <Text style={styles.heroStatValue}>{Math.max(materials.length - lowStockCount, 0)}</Text>
        ...
        <View style={styles.notice}>
          ...
          <View style={styles.noticeContent}>
            ...
            <Text style={styles.noticeText}>
              Copie .env.example para .env e informe a URL e a chave pública do projeto.
            </Text>
          </View>
        </View>
      ) : null}
      ...
      {error ? (
        <Pressable onPress={onRefresh} style={styles.errorNotice}>
          ...
          <View style={styles.noticeContent}>
            ...
            <Text numberOfLines={2} style={styles.noticeText}>{error} Toque para tentar novamente.</Text>
          </View>
        </Pressable>
      ) : null}
      ...
      <View style={styles.searchWrap}>
        ...
        />
        {search ? (
          <Pressable accessibilityLabel="Limpar pesquisa" onPress={() => setSearch('')}>
          ...
          loading ? (
            ...
            <View style={styles.empty}>
              <View style={styles.emptyIcon}>
                <Ionicons color={colors.primary} name="file-tray-outline" size={34} />
              </View>
              <Text style={styles.emptyTitle}>
                {materials.length ? 'Nenhum resultado' : 'Seu estoque está vazio'}
              </Text>
...
const styles = StyleSheet.create({
  ...
  brandOverline: { color: colors.accent, fontSize: 9, fontWeight: '800', letterSpacing: 1.25 },
  ...
  heroLabel: { color: '#BFD3CB', fontSize: 10, fontWeight: '800', letterSpacing: 1.1 },
  heroValue: { color: colors.white, fontSize: 29, fontWeight: '900', marginTop: 7 },
  heroIcon: {
    alignItems: 'center',
  ...
  heroDivider: { backgroundColor: 'rgba(255,255,255,0.13)', height: 1, marginVertical: 17 },
  ...
  noticeText: { color: colors.textMuted, fontSize: 12, lineHeight: 18, marginTop: 3 },
  searchWrap: {
    alignItems: 'center',
    backgroundColor: colors.surface,
    borderColor: colors.border,
