...
describe('serviço de materiais', () => {
  ...
  it('cadastra e devolve o material criado', async () => {
    ...
    expect(insert).toHaveBeenCalledWith([
      expect.objectContaining({ ...input, criado_em: expect.any(String), atualizado_em: expect.any(String) }),
    ]);
  });
  ...
  it('edita o material selecionado', async () => {
    ...
    await expect(editarMaterial(1, { ...input, estoque: 30 })).resolves.toEqual(updated);
    expect(eq).toHaveBeenCalledWith('id', 1);
    expect(update).toHaveBeenCalledWith(
      expect.objectContaining({ ...input, estoque: 30, atualizado_em: expect.any(String) }),
    );
  });
  ...
  it('propaga erros retornados pelo Supabase', async () => {
    const order = vi.fn().mockResolvedValue({ data: null, error: { message: 'Falha remota' } });
